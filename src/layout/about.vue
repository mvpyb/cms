        ...
        <div class="fl about-wrapper-left">
          ...
          >
            <el-menu-item
              ...
              index="2"
              class="side-bar-item"
              :class="{ 'active' : $route.path == `/about/${nav.path}` || curretActivepath == `/about/${nav.path}` }"
            >
...
export default {
  ...
  computed : {
    ...
    getCurrentNav() {
      // const currentRoutes = this.$router.options.routes.find( item => item.path == '/about' )
      const currentRoutes = this.routes.find( item => item.path == '/about' )
      if ( currentRoutes ) {
  ...
  mounted() {
    // console.log( 'ss', this.$router.options.routes )
    // console.log( 'about 当前路由', this.$route.path )
    // console.log( 'about routes', this.routes )
  },
  methods : {
    changeRoute( data ) {
      this.$router.push( `/about/${data.path}` )
    }
  }
}
</script>
