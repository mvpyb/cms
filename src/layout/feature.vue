        ...
        <div class="fl about-wrapper-left">
          ...
          >
            <el-menu-item
              ...
              index="2"
              class="side-bar-item"
              :class="{ 'active' : $route.path == `/feature/${nav.path}` || curretActivepath == `/feature/${nav.path}` }"
            >
...
export default {
  ...
  computed : {
    ...
    getCurrentNav() {
      // const currentRoutes = this.$router.options.routes.find( item => item.path == '/feature' )
      const currentRoutes = this.routes.find( item => item.path == '/feature' )
      if ( currentRoutes ) {
  ...
  methods : {
    changeRoute( data ) {
      this.$router.push( `/feature/${data.path}` )
    }
  }
}
</script>
