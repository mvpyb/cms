        ...
        <div class="fl about-wrapper-left">
          ...
          >
            <el-menu-item
              ...
              index="2"
              class="side-bar-item"
              :class="{ 'active' : $route.path == `/dynamic/${nav.path}` || curretActivepath == `/dynamic/${nav.path}` }"
            >
...
export default {
  ...
  mounted() {
    console.log( 'currentRoutes', this.getCurrentNav )
  },
  methods : {
    changeRoute( data ) {
      this.$router.push( `/dynamic/${data.path}` )
    }
  }
}
</script>
